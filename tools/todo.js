...
const sel = items.filter((i) => !i.vi && i.line >= from && i.line <= to).slice(0, limit);
sel.forEach((i) => console.log(`${i.id}\t${i.line}\t${i.ctx}\t${JSON.stringify(i.zh)}`));
console.log(`--- ${sel.length} chuoi (con lai trong khoang: ${items.filter((i) => !i.vi && i.line >= from && i.line <= to).length}) ---`);
